...
readonly TEST_NAME="Preconditioning_Curve_8k_Compressed_Read-Write"
...
for PASS in {1..360};
do
	$FIO --output-format=json --name=job --filename=$1 --iodepth=$OIO --numjobs=$THREADS --bs=8192 --ioengine=libaio --rw=randrw --rwmixread=70 --group_reporting --runtime=60 --time_based --direct=1 --randrepeat=0 --norandommap --thread --refill_buffers --buffer_compress_percentage=50 --output=${TEST_NAME}/results/fio_pass=${PASS}.json
	clear
